...
const Controls: React.FC<ControlsProps> = ({
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 bg-[#050505]/90 backdrop-blur-xl border-t border-white/10 animate-slide-up pb-[env(safe-area-inset-bottom)]">
      ...
      <div className="max-w-3xl mx-auto px-6 h-24 flex items-center justify-between gap-4">
        
        {/* Play/Pause */}
        <button 
            onClick={onPlayPause}
            disabled={isAudioLoading}
            className={`flex-shrink-0 flex items-center justify-center w-12 h-12 rounded-full transition-all duration-300 shadow-[0_0_15px_rgba(255,255,255,0.1)] hover:shadow-[0_0_20px_rgba(255,255,255,0.2)] hover:scale-105 active:scale-95 ${
                isAudioLoading 
        ...
        {/* Time Info (Mobile Hidden / Desktop Visible) */}
        <div className="text-xs font-mono text-cube-text-muted hidden md:block w-20 text-center">
            {formatTime(currentTime)} / {formatTime(duration)}
        </div>

        {/* Control Group */}
        <div className="flex items-center bg-white/5 border border-white/10 rounded-full p-1 shadow-lg backdrop-blur-md flex-1 md:flex-none justify-center">
            
            {/* Subtitle Toggle */}
            <button 
                onClick={onToggleSubtitle}
                className="group flex items-center justify-center gap-2 px-4 py-2 rounded-full hover:bg-white/10 transition-all duration-200"
            >
            ...
            <button 
                onClick={onComplete}
                className="group flex items-center gap-2 px-4 py-2 rounded-full hover:bg-cube-accent text-zinc-300 hover:text-white transition-all duration-200"
            >
